...
-- HR Dashboard Advanced Parameter & KPI System
-- =====================================================

-- Fix the circular dependency between employees and departments
-- Step 1: Remove the manager_id constraint temporarily if it exists
ALTER TABLE public.departments DROP CONSTRAINT IF EXISTS departments_manager_id_fkey;

-- Step 2: Create the evaluation parameters table
CREATE TABLE IF NOT EXISTS public.evaluation_parameters (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    parameter_id text UNIQUE NOT NULL, -- e.g., 'COG_01', 'SOC_10'
    name text NOT NULL,
    category text NOT NULL,
    definition text NOT NULL,
    relevance_summary text,
    behavioral_anchors jsonb NOT NULL, -- stores the 1-5 scale anchors
...
CREATE TABLE IF NOT EXISTS public.employee_parameter_ratings (
    ...
    rater_id uuid REFERENCES public.employees(id), -- who provided this rating
    rater_type text NOT NULL CHECK (rater_type IN ('self', 'manager', 'peer', 'system')),
    evidence_text text, -- behavioral example or justification
...
CREATE TABLE IF NOT EXISTS public.advanced_kpis (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    kpi_code text UNIQUE NOT NULL, -- 'FRLP', 'IV', 'CHI'
    name text NOT NULL,
    description text,
    formula_expression text NOT NULL, -- mathematical formula using parameter IDs
    parameter_weights jsonb NOT NULL, -- stores weights for each parameter
...
CREATE TABLE IF NOT EXISTS public.employee_advanced_kpi_values (
    ...
    component_scores jsonb NOT NULL, -- stores individual parameter contributions
    ...
    metadata jsonb DEFAULT '{}',
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()),
    PRIMARY KEY (employee_id, kpi_code, period_start),
    INDEX idx_employee_kpi_period (employee_id, kpi_code, period_start)
);

-- Create parameter audit trail
...
-- Create 360-degree feedback requests table
CREATE TABLE IF NOT EXISTS public.feedback_requests (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    employee_id uuid NOT NULL REFERENCES public.employees(id),
    requested_by uuid NOT NULL REFERENCES public.users(id),
    request_type text NOT NULL CHECK (request_type IN ('360', 'peer', 'upward', 'self')),
    parameters_to_rate text[] NOT NULL, -- array of parameter IDs
    target_raters uuid[] NOT NULL, -- array of employee IDs
...
-- Create ML insights table for AI-powered analysis
CREATE TABLE IF NOT EXISTS public.employee_ai_insights (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    employee_id uuid NOT NULL REFERENCES public.employees(id),
    insight_type text NOT NULL, -- 'risk_assessment', 'potential_prediction', 'development_recommendation'
...
-- Fix the department-employee relationship properly
-- Re-add the manager constraint with proper handling
...
CREATE INDEX IF NOT EXISTS idx_employees_department_manager 
ON public.employees(department_id, manager_id);

-- Add useful views for reporting
CREATE OR REPLACE VIEW public.employee_latest_ratings AS
SELECT DISTINCT ON (employee_id, parameter_id) 
    employee_id,
    parameter_id,
    rating_value,
    rater_type,
    ...
    rating_period_end,
    created_at
FROM public.employee_parameter_ratings
ORDER BY employee_id, parameter_id, rating_period_end DESC, created_at DESC;

CREATE OR REPLACE VIEW public.employee_parameter_summary AS
SELECT 
    e.id as employee_id,
    e.name as employee_name,
    e.department_id,
    d.name as department_name,
    COUNT(DISTINCT epr.parameter_id) as parameters_rated,
    AVG(epr.rating_value) as average_rating,
    AVG(epr.confidence_score) as average_confidence,
    MAX(epr.rating_period_end) as latest_rating_period
FROM public.employees e
LEFT JOIN public.employee_parameter_ratings epr ON e.id = epr.employee_id
LEFT JOIN public.departments d ON e.department_id = d.id
WHERE e.is_active = true
GROUP BY e.id, e.name, e.department_id, d.name;

-- =====================================================
-- DATA INSERTION: 35 EVALUATION PARAMETERS
-- =====================================================

-- Insert the 35 evaluation parameters
...
-- Insert remaining parameters (PER and ETH categories)
INSERT INTO public.evaluation_parameters (parameter_id, name, category, definition, relevance_summary, behavioral_anchors) VALUES 
...
-- =====================================================
-- INSERT ADVANCED KPIs
-- =====================================================

-- Insert the 3 advanced KPIs with their formulas
...
-- =====================================================
-- CREATE FUNCTIONS FOR KPI CALCULATIONS
-- =====================================================

-- Function to calculate advanced KPI for an employee
...
BEGIN
    ...
    LOOP
        ...
        -- Use default value if no rating found
        IF param_value IS NULL THEN
            param_value := 3.0; -- Default neutral rating
        END IF;
    ...
    IF total_weight > 0 THEN
        calculated_score := calculated_score / total_weight * 5.0; -- Scale to 1-5
    END IF;
...
-- Function to bulk calculate KPIs for all employees
CREATE OR REPLACE FUNCTION recalculate_all_advanced_kpis(
    p_period_start date DEFAULT CURRENT_DATE - INTERVAL '90 days',
    p_period_end date DEFAULT CURRENT_DATE
) RETURNS text AS $$
DECLARE
...
BEGIN
    ...
    LOOP
        -- Loop through all active KPIs
        FOR kpi_record IN 
            SELECT kpi_code FROM advanced_kpis WHERE is_active = true
        LOOP
            PERFORM calculate_advanced_kpi(emp_record.id, kpi_record.kpi_code, p_period_start, p_period_end);
            calculation_count := calculation_count + 1;
        END LOOP;
    END LOOP;
    
    RETURN format('Calculated %s KPI values for period %s to %s', calculation_count, p_period_start, p_period_end);
END;
$$ LANGUAGE plpgsql;
...
-- Insert sample parameter ratings for existing employees (if any exist)
-- This is just for demonstration - in production, ratings would come from actual evaluations

INSERT INTO employee_parameter_ratings (employee_id, parameter_id, rating_value, rater_type, rating_period_start, rating_period_end)
SELECT 
    e.id,
    'COG_01',
    3.0 + (random() * 2), -- Random rating between 3-5
    'system',
...
GRANT EXECUTE ON FUNCTION recalculate_all_advanced_kpis TO anon, authenticated;

-- =====================================================
-- COMPLETION MESSAGE
-- =====================================================

SELECT 'Database schema update completed successfully. 35 parameters and 3 advanced KPIs have been implemented.' as status; 